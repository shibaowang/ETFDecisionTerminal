...
## TASK-189 Manual Entry Schema Implementation Authorization Gate

TASK-189 adds a gate-only authorization boundary for future manual entry schema
implementation. It follows TASK-188 and defines readiness rules for an
independent migration file, migration id, rollback / forward-fix policy,
compatibility, idempotency, manual transaction candidate fields, manual cash
movement candidate fields, and audit / rollback / privacy categories.

TASK-189 does not modify `migrations/001_initial_schema.sql`, does not add a
migration, does not add a schema file, does not implement repository writes,
does not execute SQL, does not write SQLite, and does not modify
DataServiceActions, TASK-178 validation production code, TASK-185 repository
scaffold code, QML, startup, replay, broker, strategy, or automatic trading
...
independent migration file, and must not directly edit
`migrations/001_initial_schema.sql`. Future repository implementation and
DataService write implementation must remain separate tasks and must wait until
schema gaps are resolved or a lossless mapping is separately authorized.
...
## TASK-216 Manual Entry Replay Fixture Files Scaffold Authorization Gate

TASK-216 adds a gate-only replay fixture files scaffold authorization. It
defines future scaffold directory, file set, fixture index, placeholder
content, JSON schema, static validation, privacy, no-runtime-dependency, and
out-of-scope boundaries for manual entry replay fixture file scaffolding.

TASK-216 does not create replay fixture files, does not create replay fixture
JSON, does not create fixture index files, does not create seed DB or runtime
fixture data, does not implement parser / validator work, does not implement
replay, does not call AccountingEngine replay, does not modify production code,
production QML, DataServiceActions, repositories, or migrations, does not write
audit / ledger / snapshot rows, does not implement backup/export/restore, does
not connect broker, network, credentials, or endpoint, and does not place real
...
## TASK-247 Manual Entry Replay AccountingEngine Bridge Regression Matrix Gate

TASK-247 adds a test-only regression matrix gate for the TASK-246 manual entry
replay AccountingEngine bridge contract shell. The CTest
`shell_accounting_manual_entry_replay_accountingengine_bridge_regression_matrix`
verifies bridge CLI support, positive contract output, sanitized failure
output, fail-closed behavior for unsafe summaries, exact changed-path
allowlists, fixture immutability, no AccountingEngine code drift, no
AccountingEngine replay calls, no production bridge, no runtime writes, no
read model refresh, and no broker / network / credential / endpoint drift.

TASK-247 is test-only and regression-matrix-only. TASK-247 does not call
