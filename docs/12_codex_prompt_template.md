...
## EPIC-289 real daily-use data dashboard boundary

- EPIC-289 may update the ShellAccounting daily-use QML panel, ShellServices
  Presenter read-only mapping, DataService read-only action, MarketEngine
  startup public-market refresh boundary, local daily-use scripts, docs 401
...
- Startup public-market refresh is bounded to exact hosts
  ...
  breaker, no same-host concurrency, visible status, and fixture-only tests.
- Do not add manual market refresh controls to the daily-use UI. Do not add
  background polling, hidden network calls, broker, credentials, endpoints,
  real order placement, automatic trading, TradeDraft generation, strategy
  execution, migrations, or production DB destructive mutation.

## EPIC-288 global Chinese UI leak visual acceptance boundary

- EPIC-288 may update QML under `apps/ETFDecisionShell/qml/**/*.qml`,
  `docs/400_global_chinese_ui_readability_trial_feedback_fix.md`, README indexes,
  the prompt template, and the existing global Chinese readability / Dashboard /
  local-trial static acceptance tests.
- Preserve QML `objectName` values, Presenter calls, action names, schema keys,
  ...
  English; default visible explanatory text must be Chinese-first or
  Chinese-annotated.
- Right-side placeholder titles, metric names, action hints, ReadOnlyData
  onboarding text, and status enum values must not leak bare English into the
  default visual trial screens.
- Raw JSON / sample payload / fixture details must stay collapsed by default
  and be available only behind Chinese developer-detail controls.
- Do not change business behavior, DataAccess writes, DataServiceApi write
  semantics, replay semantics, market provider semantics, strategy semantics,
  TradeDraft semantics, OTCMap semantics, migrations, broker / credential /
...
## TASK-221 Manual Entry Replay Negative Fixtures Scaffold Authorization Gate Prompt Rule

Future manual entry replay negative fixture scaffold file work must reference
...
TASK-221 authorizes future negative fixture scaffold policy only. TASK-221 does
not authorize creation of negative fixture files. TASK-221 does not authorize
creation of the negative fixture directory. TASK-221 does not authorize
negative validator implementation. TASK-221 does not authorize parser, loader,
or reader implementation. TASK-221 does not authorize replay implementation.
TASK-221 does not authorize AccountingEngine replay calls. TASK-221 does not
authorize runtime SQL / SQLite read/write. TASK-221 does not authorize audit /
ledger / snapshot writes. TASK-221 does not authorize broker, network,
credentials, endpoint, real order placement, or automatic trading.

Recommended next task: TASK-222 manual entry replay negative fixture scaffold
files gate.

## TASK-222 Manual Entry Replay Negative Fixture Scaffold Files Gate Prompt Rule

Future manual entry replay negative fixture static validator work must
reference
...
TASK-222 authorizes creation of test-only negative fixture scaffold JSON files
only. TASK-222 does not authorize negative validator implementation. TASK-222
does not authorize parser, loader, or reader implementation. TASK-222 does not
authorize replay implementation. TASK-222 does not authorize AccountingEngine
replay calls. TASK-222 does not authorize runtime SQL / SQLite read/write.
TASK-222 does not authorize audit / ledger / snapshot writes. TASK-222 does
...
## TASK-224 Manual Entry Replay Negative Fixture Static Validator Implementation Gate Prompt Rule

Future manual entry replay negative fixture static validator regression matrix
work must reference
...
TASK-224 authorizes a test-only negative fixture static validator only. The
validator may read the TASK-222 negative fixture index, ten `NEG_MRF*.json`
scaffold files, and the TASK-217 positive fixture index for source mapping.
TASK-224 does not authorize production validator, parser, loader, reader, or
replay implementation. TASK-224 does not authorize AccountingEngine replay
calls. TASK-224 does not authorize runtime SQL / SQLite read/write. TASK-224
does not authorize audit / ledger / snapshot writes. TASK-224 does not
...
## TASK-231 Manual Entry Replay Test-Only Dry-Run Harness Implementation Gate Prompt Rule

Future manual entry replay dry-run harness regression matrix work must
reference
...
TASK-231 implements a test-only dry-run harness that reads only authorized
manual entry replay fixture metadata and fixture JSON, emits a sanitized
dry-run summary, and preserves fixture immutability. TASK-231 does not
authorize production parser / loader / reader behavior, replay implementation,
AccountingEngine replay calls, runtime SQL / SQLite read/write, audit / ledger
/ snapshot writes, production integration, QML, DataServiceActions,
repositories, migrations, broker, network, credentials, endpoint, real order
placement, automatic trading, or real position / cash / PnL calculation.
...
## TASK-240 Manual Entry Replay AccountingEngine Adequacy Review Implementation Prompt Rule

Future manual entry replay AccountingEngine bridge, runtime integration, replay
implementation, read model refresh, production parser / loader / reader work,
or AccountingEngine replay calls must reference
...
TASK-240 implements the AccountingEngine adequacy review gate only. It is a
test-only, deterministic, no-write static gate. TASK-240 does not implement an
AccountingEngine bridge, does not call AccountingEngine replay, does not modify
AccountingEngine code, does not implement production replay, does not authorize
runtime SQL / SQLite read/write, audit / ledger / snapshot writes, read model
refresh, real position / cash / PnL calculation, production integration, QML,
DataServiceActions, repositories, migrations, broker, network, credentials,
endpoint, real order placement, or automatic trading.
...
## TASK-243 Manual Entry Replay AccountingEngine Adequacy Review CI Closeout Prompt Rule

Future manual entry replay AccountingEngine adequacy review phase closeout,
handoff, bridge authorization, replay implementation, runtime integration, read
model refresh, production parser / loader / reader work, or AccountingEngine
...
TASK-243 adds a review-only CI closeout gate for AccountingEngine adequacy
review. TASK-243 does not call AccountingEngine replay, does not implement
AccountingEngine bridge, does not implement replay, does not modify
AccountingEngine code, does not authorize runtime SQL / SQLite read/write,
audit / ledger / snapshot writes, read model refresh, production integration,
broker, network, credentials, endpoint, real order placement, or automatic
trading. Future bridge, replay, read model refresh, production integration, and
...
## TASK-244 Manual Entry Replay AccountingEngine Adequacy Review Phase Closeout Handoff Prompt Rule

Future manual entry replay AccountingEngine bridge authorization, bridge
implementation, replay implementation, runtime integration, read model refresh,
production parser / loader / reader work, or AccountingEngine replay calls must
reference
...
TASK-244 closes out the AccountingEngine adequacy review phase only. TASK-244
does not call AccountingEngine replay, does not implement AccountingEngine
bridge, does not implement replay, does not modify AccountingEngine code, does
not authorize runtime SQL / SQLite read/write, audit / ledger / snapshot
writes, read model refresh, production integration, broker, network,
credentials, endpoint, real order placement, or automatic trading. Future
bridge, replay, read model refresh, production integration, and AccountingEngine
replay calls must be separately authorized.
