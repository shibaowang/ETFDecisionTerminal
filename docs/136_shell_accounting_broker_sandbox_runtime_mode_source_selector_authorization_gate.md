...
TASK-169 is an authorization gate only. It defines the future boundary for a sandbox runtime mode source selector; TASK-169 does not implement that selector. TASK-170 later adds a direct-test-only selector scaffold, but it still does not enable sandbox runtime and still does not modify `DataServiceActions`.

The TASK-168 sandbox runtime mode source scaffold remains direct-test-only. The TASK-170 selector scaffold is also direct-test-only. The default runtime source remains disabled-only; sandbox runtime remains disabled.
...
- TASK-170 adds `shellAccountingBrokerRuntimeModeSourceForMode` as a direct-test selector scaffold only.
...
Runtime wiring for the selector must be implemented in a separate TASK. The TASK-170 scaffold may only choose between explicitly authorized sources, initially the disabled source and the TASK-168 sandbox scaffold source, and it must fail closed for unknown, blank, unsupported, paper, and real modes.
...
TASK-169 adds no runtime mode source selector implementation. TASK-170 adds only the direct-test selector scaffold. Neither task adds sandbox runtime enablement, payload mode source, QML mode source, config source, environment source, command-line source, file source, database source, or secret-store source.
