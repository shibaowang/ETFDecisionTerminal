...
struct ActionServiceHost::EnvelopeParseResult final {
    bool success = false;
...
ActionServiceHost::ActionServiceHost(etfdt::service_runtime::ActionDispatcher& dispatcher)
...
ActionServiceHost::~ActionServiceHost()
{
    close();
}

etfdt::transport::TransportResult<bool> ActionServiceHost::listen(const std::string& socketName)
{
    return server_.listen(socketName);
}

void ActionServiceHost::close()
{
    server_.close();
}

bool ActionServiceHost::isListening() const noexcept
{
    return server_.isListening();
}

std::size_t ActionServiceHost::clientCount() const noexcept
{
    return server_.clientCount();
}

void ActionServiceHost::handleTransportMessage(
...
ActionServiceHost::EnvelopeParseResult ActionServiceHost::parseEnvelope(
...
etfdt::protocol::ProtocolResponse ActionServiceHost::makeParseErrorResponse(
...
void ActionServiceHost::sendResponse(
    etfdt::transport::LocalSocketServer::ClientId clientId,
