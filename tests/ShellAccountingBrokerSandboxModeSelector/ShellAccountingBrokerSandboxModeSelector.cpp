...
namespace etfdt::tests::shell_accounting_broker_sandbox_mode_selector {
namespace {
...
std::string docs122Text(const std::filesystem::path& root)
{
    return readTextFile(root / "docs" / "122_shell_accounting_broker_sandbox_mode_selector.md");
}

std::string docs123Text(const std::filesystem::path& root)
{
    return readTextFile(root / "docs" / "123_shell_accounting_broker_sandbox_mode_selector_test_plan.md");
}

std::string task162DocsText(const std::filesystem::path& root)
{
    return docs122Text(root) + "\n" + docs123Text(root);
}

std::string task160And161DocsText(const std::filesystem::path& root)
{
...
std::vector<std::string> qmlPresenterExposureTokens()
{
    return {"ShellAccountingBrokerOrderPortModeSelector", "shellAccountingBrokerOrderPortForMode",
            "BrokerSandboxOrderPort", "brokerSandboxAdapter", "brokerCredential", "brokerEndpoint"};
}
