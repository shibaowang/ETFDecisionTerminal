...
int main(int argc, char* argv[])
{
    ...
    expectTrue(
        result.observedAction == "portfolio.pnl.summary",
        "portfolio pnl guard wrapper called");
    expectTrue(
        result.state == etfdt::shell_services::ShellAccountingViewState::Unavailable,
        "portfolio pnl guard payload maps to Unavailable");
    expectTrue(
        etfdt::shell_services::tests::controllerIssuesContainCode(
            result.issues,
            "PORTFOLIO_PNL_SUMMARY_NOT_AVAILABLE"),
        "portfolio pnl unavailable issue visible");
