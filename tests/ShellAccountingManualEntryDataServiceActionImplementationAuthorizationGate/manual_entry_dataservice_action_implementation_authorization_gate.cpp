...
void testNoDataAccessWriteRepository(const Harness& h)
{
    ...
    // DataAccess-only direct-test repository; DataService implementation write
    // wiring remains forbidden by this authorization gate.
...
void testNoNewMigration(const Harness& h)
{
    for (const auto& file : filesUnder(h.root / "migrations")) {
        ...
        require(
            ...
            "TASK-181 must not add unauthorized migration " + file.string());
    }
}
