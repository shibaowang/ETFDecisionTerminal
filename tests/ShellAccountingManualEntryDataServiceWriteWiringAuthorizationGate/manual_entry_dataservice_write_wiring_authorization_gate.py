...
def diff_text_excluding_paths(root: Path, include_paths: tuple[str, ...], excluded_paths: set[str]) -> str:
    changed = subprocess.run(
        ["git", "diff", "--name-only", "main", "--", *include_paths],
        cwd=root,
        check=True,
        capture_output=True,
        text=True,
    ...
    for line in changed.stdout.splitlines():
        ...
        if not path or path in excluded_paths:
            continue
        diffs.append(diff_text(root, path))
    return "\n".join(diffs)
...
def main() -> int:
    ...
    require(not any(path.startswith("libs/AccountingEngine/") and path not in EPIC_276_EXACT_PATHS for path in changes), "TASK-197 must not modify AccountingEngine")
    require(not any(path.startswith("libs/StrategyEngine/") for path in changes), "TASK-197 must not modify StrategyEngine")
    require(not any(path.startswith("libs/MarketEngine/") and path not in EPIC_280_EXACT_PATHS for path in changes), "TASK-197 must not modify MarketEngine")
    ...
    runtime_diff = added_lines(
        diff_text_excluding_paths(
            root,
            ("libs", "apps"),
            TASK_259_READONLY_CLIENT_ADAPTER_PRODUCTION_PATHS | EPIC_278_EXACT_PATHS | EPIC_289_FIX_EXACT_PATHS,
        )
    )
    for token in [
        "INSERT INTO",
