...
def main() -> int:
    ...
    require(not any(path.startswith("libs/MarketEngine/") and path not in EPIC_280_EXACT_PATHS for path in changes), "TASK-199 must not modify MarketEngine")
    ...
    runtime_added = added_lines(
        ...
        *[
            f":(exclude){path}"
            for path in EPIC_278_EXACT_PATHS
            if path not in EPIC_287_GLOBAL_CHINESE_UI_READABILITY_EXACT_PATHS
        ],
        *[f":(exclude){path}" for path in EPIC_289_FIX_EXACT_PATHS],
    )
    for token in [
        "INSERT INTO",
        "UPDATE ",
