...
def main() -> int:
    ...
    require(not any(path.startswith("libs/AccountingEngine/") and path not in EPIC_276_EXACT_PATHS for path in changes), "TASK-199 must not modify AccountingEngine")
    ...
    task265_excel_vba_import_persistence_runtime_paths = {
        "libs/DataAccess/CMakeLists.txt",
        "libs/DataAccess/include/DataAccess/ShellAccountingExcelVbaImportManualEntryPersistenceRepository.h",
        "libs/DataAccess/include/DataAccess/ShellAccountingManualTransactionRepository.h",
        "libs/DataAccess/include/DataAccess/ShellAccountingManualCashMovementRepository.h",
    ...
    runtime_added = added_lines(
        root,
        "apps",
        "libs",
        "migrations",
        *[f":(exclude){path}" for path in TASK_259_READONLY_CLIENT_ADAPTER_PRODUCTION_PATHS],
        *[f":(exclude){path}" for path in task265_excel_vba_import_persistence_runtime_paths],
    )
    for token in [
        "INSERT INTO",
        "UPDATE ",
