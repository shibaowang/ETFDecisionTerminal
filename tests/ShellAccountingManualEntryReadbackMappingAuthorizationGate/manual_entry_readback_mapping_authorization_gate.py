...
def main() -> int:
    ...
    for prefix in forbidden_prefixes:
        require(not any(path.startswith(prefix) and path not in (TASK_257_EXACT_PATHS | EPIC_276_EXACT_PATHS | EPIC_277_ALLOWED_CHANGED_PATHS) for path in changes), f"TASK-206 must not change {prefix}")
    for path in changes:
    ...
    production_diff = added_lines(
        diff_text_excluding_paths(
            root,
            ("apps", "libs", "migrations"),
