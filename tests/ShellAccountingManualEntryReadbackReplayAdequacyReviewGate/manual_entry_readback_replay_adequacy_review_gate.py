...
def main() -> int:
    ...
    production_diff = added_lines(diff_text(root, "apps", "libs", "migrations", *[f":(exclude){path}" for path in TASK_259_READONLY_CLIENT_ADAPTER_PRODUCTION_PATHS], *[f":(exclude){path}" for path in EPIC_278_EXACT_PATHS]))
    for token in [
        "INSERT ",
        "UPDATE ",
        "DELETE ",
