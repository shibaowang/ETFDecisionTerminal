...
def validate_static_boundaries(gate: Gate, root: Path) -> None:
    ...
    for path in ["apps", "libs", "migrations", "libs/AccountingEngine", POSITIVE_DIR.as_posix(), NEGATIVE_DIR.as_posix()]:
        diff_paths = git_lines(root, "diff", "--name-only", "main", "--", path)
        if path == "apps":
            gate.require(diff_paths <= ({"apps/ETFDecisionShell/qml/pages/ShellAccountingReadOnlyPage.qml"} | EPIC_287_GLOBAL_CHINESE_UI_READABILITY_EXACT_PATHS | EPIC_289_FIX_EXACT_PATHS), f"{path} remains exact TASK-261 QML panel only")
        elif path == "libs":
            gate.require(diff_paths <= (TASK_257_EXACT_PATHS | EPIC_276_EXACT_PATHS | EPIC_277_ALLOWED_CHANGED_PATHS), f"{path} remains exact TASK-257 parser boundary only")
        elif path == "libs/AccountingEngine":
            gate.require(diff_paths <= EPIC_276_EXACT_PATHS, f"{path} remains exact EPIC-276 read-only replay only")
        else:
