...
ALLOWED_CHANGED_PATHS = {
    ...
    "tests/ShellAccountingManualEntryReplayAccountingEngine"
    "Bridge/manual_entry_replay_accountingengine_"
    "bridge.py",
...
def validate_future_bridge_directory(gate: Gate, root: Path) -> None:
    bridge_dir = root / FUTURE_BRIDGE_DIR
    if not bridge_dir.exists():
        gate.require(True, "future AccountingEngine bridge test directory may be absent before TASK-246")
        return

    allowed_files = {
        (FUTURE_BRIDGE_DIR / "CMakeLists.txt").as_posix(),
        (FUTURE_BRIDGE_DIR / ("manual_entry_replay_accountingengine_" + "bridge.py")).as_posix(),
    }
    observed_files = {
    ...
    gate.require(bridge_dir.is_dir(), "TASK-246 AccountingEngine bridge path is a test-only directory")
    gate.require(observed_files == allowed_files, "TASK-246 bridge directory contains only exact test-only files")
    gate.require(not observed_directories, "TASK-246 bridge directory has no nested runtime integration directory")
...
def main() -> int:
    ...
    for allowed_test_only_bridge_path in [
        FUTURE_BRIDGE_DIR.as_posix() + "/CMakeLists.txt",
        FUTURE_BRIDGE_DIR.as_posix() + "/manual_entry_replay_accountingengine_" + "bridge.py",
    ]:
        task_gate_scan_text = task_gate_scan_text.replace(allowed_test_only_bridge_path, "")
    for token in FORBIDDEN_EXECUTABLE_TOKENS:
