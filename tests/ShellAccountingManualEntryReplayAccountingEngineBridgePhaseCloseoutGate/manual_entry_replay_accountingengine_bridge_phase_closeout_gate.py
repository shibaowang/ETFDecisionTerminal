...
def validate_changed_paths(gate: Gate, root: Path) -> set[str]:
    ...
    for path in ["apps", "libs", "migrations", "libs/AccountingEngine", POSITIVE_DIR.as_posix(), NEGATIVE_DIR.as_posix()]:
        ...
        if path == "apps":
            gate.require(diff_paths <= ({"apps/ETFDecisionShell/qml/pages/ShellAccountingReadOnlyPage.qml"} | EPIC_287_GLOBAL_CHINESE_UI_READABILITY_EXACT_PATHS), f"{path} diff exact TASK-261 QML panel only")
        elif path == "libs":
