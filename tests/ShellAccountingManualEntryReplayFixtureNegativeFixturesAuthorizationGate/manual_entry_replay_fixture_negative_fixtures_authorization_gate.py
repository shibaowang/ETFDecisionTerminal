...
def main() -> int:
    ...
    if changes:
        gate.require(
            ...
            "TASK-220 changed paths must match the exact negative fixtures authorization allowlist",
        )
    ...
    negative_json_changes = sorted(
        path for path in changes if "manual_entry_replay_negative" in path or path.startswith("tests/fixtures/") and "NEG_MRF" in path
    )
