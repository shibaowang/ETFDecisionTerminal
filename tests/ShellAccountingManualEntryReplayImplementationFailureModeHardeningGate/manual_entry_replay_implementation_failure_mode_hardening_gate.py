...
TASK_236_DIR = Path("tests/ShellAccountingManualEntryReplayImplementationFailureModeHardeningGate")
TASK_236_CMAKE = TASK_236_DIR / "CMakeLists.txt"
TASK_236_GATE = TASK_236_DIR / "manual_entry_replay_implementation_failure_mode_hardening_gate.py"
TASK_236_CTEST = "shell_accounting_manual_entry_replay_implementation_failure_mode_hardening"
...
ALLOWED_CHANGED_PATHS = {
    ...
    "tests/ShellAccountingManualEntryReplayAccountingEngineRuntimeIntegrationAuthorizationGate/manual_entry_replay_accountingengine_runtime_integration_authorization_gate.py",
    TASK_236_DOC.as_posix(),
    TASK_236_PLAN.as_posix(),
    ...
    "tests/ShellAccountingManualEntryReplayAccountingEngineAdequacyReviewPhaseCloseoutGate/manual_entry_replay_accountingengine_adequacy_review_phase_closeout_gate.py",
    "tests/CMakeLists.txt",
    TASK_236_CMAKE.as_posix(),
    TASK_236_GATE.as_posix(),
...
def validate_changed_paths(gate: Gate, root: Path) -> set[str]:
    ...
    gate.require(not unexpected, "TASK-236 changed unauthorized paths: " + ", ".join(unexpected))
...
def validate_docs(gate: Gate, root: Path) -> None:
    gate.require((root / TASK_236_DOC).exists(), "docs/268 exists")
    gate.require((root / TASK_236_PLAN).exists(), "docs/269 exists")
    doc268 = read(root / TASK_236_DOC)
    doc269 = read(root / TASK_236_PLAN)
    for section in [
        "## Purpose",
        ...
        gate.contains(doc268, section, "docs/268")
    for token in [
        ...
        "TASK-236 does not modify implementation success semantics.",
        "TASK-236 does not modify dry-run harness business semantics.",
        "TASK-236 does not modify validator validation behavior.",
        "TASK-236 does not implement production parser / loader / reader.",
        "TASK-236 does not implement production replay.",
        "TASK-236 does not call AccountingEngine replay.",
        "TASK-236 does not modify production code / QML / DataServiceActions / repositories / migrations.",
        "TASK-236 does not write runtime SQL / SQLite / audit / ledger / snapshot.",
        "TASK-236 does not refresh read models.",
        "TASK-236 does not calculate real position / cash / PnL.",
        "TASK-236 does not connect broker / network / credentials / endpoint.",
        "TASK-236 does not place real orders or enable automatic trading.",
        "Recommended next task: TASK-237 manual entry replay test-only implementation CI closeout gate.",
    ]:
        gate.contains(doc268, token, "docs/268")
    for section in ["## Document Purpose", "## Test Matrix", "## Required Probes", "## Go / No-Go Checklist"]:
        gate.contains(doc269, section, "docs/269")
    for token in [
        ...
        gate.contains(doc269, token, "docs/269")
    for index_path in [Path("README.md"), Path("docs/README.md"), Path("docs/12_codex_prompt_template.md")]:
        text = read(root / index_path)
        gate.contains(text, "TASK-236", index_path.as_posix())
        gate.contains(text, TASK_236_DOC.name, index_path.as_posix())
        gate.contains(text, TASK_236_PLAN.name, index_path.as_posix())
...
def validate_registration(gate: Gate, root: Path) -> None:
    ...
    gate.contains(
        tests_cmake,
        "add_subdirectory(ShellAccountingManualEntryReplayImplementationFailureModeHardeningGate)",
        "tests/CMakeLists",
    )
    gate.require((root / TASK_236_CMAKE).exists(), "TASK-236 CMakeLists exists")
    gate.require((root / TASK_236_GATE).exists(), "TASK-236 gate script exists")
    cmake = read(root / TASK_236_CMAKE)
    gate.contains(cmake, TASK_236_CTEST, "TASK-236 CMakeLists")
    gate.contains(cmake, TASK_236_GATE.name, "TASK-236 CMakeLists")
...
def validate_static_boundaries(gate: Gate, root: Path) -> None:
    for relative in [TASK_234_IMPLEMENTATION, TASK_231_HARNESS, TASK_224_VALIDATOR]:
        text = read(root / relative)
        lowered = text.lower()
        for token, message in FORBIDDEN_STATIC_TOKENS:
            haystack = lowered if token.islower() else text
            gate.require(token not in haystack, f"{relative.as_posix()} {message}")
        gate.require("ctest --test-dir build --output-on-failure" not in text, f"{relative.as_posix()} does not run full CTest")
        gate.require("AccountingEngine().replay" not in text, f"{relative.as_posix()} no AccountingEngine replay")


def run_dry_run_summary(gate: Gate, root: Path) -> dict[str, Any]:
    completed = subprocess.run(
    ...
    gate.require(completed.returncode == 0, "baseline dry-run harness exits zero")
    gate.require(completed.stderr.strip() == "", "baseline dry-run harness stderr empty")
    summary = json.loads(completed.stdout)
    gate.require(isinstance(summary, dict), "baseline dry-run summary object")
    gate.require(summary.get("schemaVersion") == DRY_RUN_SCHEMA, "baseline dry-run schema")
    gate.require(summary.get("dryRunStatus") == "ok", "baseline dry-run status ok")
    gate.require(isinstance(summary.get("entries"), list) and summary["entries"], "baseline dry-run entries present")
    return summary


def clone_summary(summary: dict[str, Any]) -> dict[str, Any]:
    return copy.deepcopy(summary)


FailureMutator = Callable[[dict[str, Any]], str | dict[str, Any]]


def mutate_entry(summary: dict[str, Any], mutator: Callable[[dict[str, Any]], None]) -> dict[str, Any]:
    entry = summary["entries"][0]
    mutator(entry)
    return summary


def failure_cases() -> list[tuple[str, FailureMutator]]:
    return [
        ("invalid dry-run summary JSON", lambda _summary: "{ invalid json Traceback SELECT password endpoint }"),
        ("wrong summary schemaVersion", lambda summary: {**summary, "schemaVersion": "wrong-schema"}),
        ("dryRunStatus not ok", lambda summary: {**summary, "dryRunStatus": "failed"}),
        ("missing entries", lambda summary: {key: value for key, value in summary.items() if key != "entries"}),
        ("entries not array", lambda summary: {**summary, "entries": {"not": "array"}}),
        (
            "duplicate fixtureId",
            lambda summary: {
                **summary,
                "entries": [summary["entries"][0], {**summary["entries"][1], "fixtureId": summary["entries"][0]["fixtureId"]}],
            },
        ),
        ("missing fixtureId", lambda summary: mutate_entry(summary, lambda entry: entry.pop("fixtureId", None))),
        ("missing source", lambda summary: mutate_entry(summary, lambda entry: entry.pop("source", None))),
        ("missing dryRunStatus", lambda summary: mutate_entry(summary, lambda entry: entry.pop("dryRunStatus", None))),
        ("missing plannedStepCount", lambda summary: mutate_entry(summary, lambda entry: entry.pop("plannedStepCount", None))),
        ("plannedStepCount negative", lambda summary: mutate_entry(summary, lambda entry: entry.update({"plannedStepCount": -1}))),
        ("replayExecuted=true", lambda summary: mutate_entry(summary, lambda entry: entry.update({"replayExecuted": True}))),
        (
            "accountingEngineCalled=true",
            lambda summary: mutate_entry(summary, lambda entry: entry.update({"accountingEngineCalled": True})),
        ),
        ("runtimeWrites=true", lambda summary: mutate_entry(summary, lambda entry: entry.update({"runtimeWrites": True}))),
        ("raw SQL token present", lambda summary: {**summary, "diagnostic": "SELECT * FROM trade_log"}),
        ("credential token present", lambda summary: {**summary, "diagnostic": "credential=secret"}),
        ("endpoint token present", lambda summary: {**summary, "diagnostic": "endpoint=https://broker.example"}),
        ("broker payload token present", lambda summary: {**summary, "diagnostic": "broker payload"}),
        ("real order id token present", lambda summary: {**summary, "diagnostic": "real order id"}),
        ("production path reference present", lambda summary: {**summary, "diagnostic": "apps/ETFDecisionShell/qml"}),
        ("DB/WAL/SHM artifact present", lambda summary: {**summary, "diagnostic": "scratch.db-wal"}),
        ("stack trace token present", lambda summary: {**summary, "diagnostic": "Traceback stack trace"}),
    ]


def write_bad_summary(path: Path, payload: str | dict[str, Any]) -> None:
...
def validate_failure_output(gate: Gate, label: str, completed: subprocess.CompletedProcess[str]) -> None:
    gate.require(completed.returncode != 0, f"failure case returns non-zero: {label}")
    combined = (completed.stdout + completed.stderr).lower()
    for token in FORBIDDEN_OUTPUT_TOKENS:
        gate.require(token not in combined, f"failure output excludes `{token}` for {label}")
    gate.require("{" in completed.stdout, f"failure stdout contains JSON object: {label}")
    failure = json.loads(completed.stdout)
    gate.require(isinstance(failure, dict), f"failure summary object: {label}")
    gate.require(failure.get("schemaVersion") == FAILURE_SCHEMA, f"failure schema correct: {label}")
    gate.require(failure.get("ok") is False, f"failure ok false: {label}")
    gate.require(failure.get("failureCount") == 1, f"failure count one: {label}")
    failures = failure.get("failures")
    gate.require(isinstance(failures, list) and len(failures) == 1, f"failure list shape: {label}")
    item = failures[0]
    gate.require(isinstance(item, dict), f"failure item object: {label}")
    gate.require(item.get("issueCode") == "REPLAY_IMPLEMENTATION_CONTRACT_VIOLATION", f"issue code stable: {label}")
    gate.require(item.get("rule") == "dry-run-summary-contract", f"failure rule stable: {label}")
    gate.require(item.get("file") == "dry-run-summary", f"failure file sanitized: {label}")
    gate.require(isinstance(item.get("message"), str) and "sanitized" in item["message"], f"message sanitized: {label}")
    encoded = json.dumps(failure, sort_keys=True).lower()
    for token in FORBIDDEN_OUTPUT_TOKENS:
        gate.require(token not in encoded, f"failure JSON excludes `{token}` for {label}")


def run_failure_cases(gate: Gate, root: Path, baseline: dict[str, Any]) -> None:
    with tempfile.TemporaryDirectory(prefix="task236_replay_failure_") as tmp:
        tmp_dir = Path(tmp)
        baseline_path = tmp_dir / "baseline_summary.json"
        baseline_path.write_text(json.dumps(baseline, sort_keys=True), encoding="utf-8")
        gate.require(baseline_path.exists(), "temporary baseline summary exists")

        for index, (label, mutator) in enumerate(failure_cases(), start=1):
            bad_summary = clone_summary(baseline)
            payload = mutator(bad_summary)
            input_path = tmp_dir / f"bad_summary_{index:02d}.json"
            write_bad_summary(input_path, payload)
            gate.require(input_path.exists(), f"temporary bad summary exists: {label}")
            completed = subprocess.run(
                [
                    sys.executable,
                    str(root / TASK_234_IMPLEMENTATION),
                    "--source-root",
                    str(root),
                    "--summary-json",
                    "--dry-run-summary-json-file",
                    str(input_path),
                ],
                cwd=root,
                check=False,
            ...
            validate_failure_output(gate, label, completed)
...
def _epic289_fix5_extend_exact_allowlists() -> None:
    ...
    for _epic289_fix5_name, _epic289_fix5_value in list(globals().items()):
        if not isinstance(_epic289_fix5_value, set):
            continue
        _epic289_fix5_upper = _epic289_fix5_name.upper()
        if "FORBIDDEN" in _epic289_fix5_upper or "DISALLOWED" in _epic289_fix5_upper:
            continue
        if (
            _epic289_fix5_name in path_allowlist_names
            or _epic289_fix5_upper.endswith("_CHANGED_PATHS")
        ):
            _epic289_fix5_value.update(EPIC_289_FIX5_EXACT_PATHS)
_epic289_fix5_extend_exact_allowlists()
def main() -> int:
    ...
    baseline = run_dry_run_summary(gate, root)
    run_failure_cases(gate, root, baseline)
    after_hashes = fixture_hashes(root)
    after_db_like = db_like_untracked_paths(root)
    changes_after = changed_paths(root)

    gate.require(before_hashes == after_hashes, "fixture JSON hashes unchanged after failure probes")
    gate.require(before_db_like == after_db_like, "no DB/WAL/SHM/SQLite files created by failure probes")
    gate.require(changes_before == changes_after, "failure probes do not change git changed-path set")
