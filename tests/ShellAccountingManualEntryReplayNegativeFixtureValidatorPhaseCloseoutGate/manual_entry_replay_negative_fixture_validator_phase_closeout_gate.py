...
def validate_changed_paths(gate: Gate, root: Path) -> set[str]:
    ...
    for prefix in FORBIDDEN_CHANGED_PREFIXES:
        gate.require(not any(path.startswith(prefix) and path not in TASK_257_EXACT_PATHS and path not in TASK_270_EXACT_PATHS for path in changes), f"no changed path under {prefix}")
...
def validate_no_runtime_boundary(gate: Gate, root: Path, changes: set[str]) -> None:
    gate.require(not any(path.startswith(("apps/", "libs/", "migrations/")) and path not in TASK_257_EXACT_PATHS and path not in TASK_270_EXACT_PATHS for path in changes), "no production changed paths")
