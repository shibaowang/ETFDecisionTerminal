...
def changed_paths(root: Path) -> set[str]:
    ...
    return {path for path in paths if path not in EPIC_277_ALLOWED_CHANGED_PATHS and path not in EPIC_278_EXACT_PATHS}

def diff_text_excluding_epic277(root: Path, *pathspecs: str) -> str:
    changed = subprocess.run(
        ["git", "diff", "--name-only", "main", "--", *pathspecs],
        cwd=root,
        check=True,
        capture_output=True,
        text=True, encoding="utf-8", errors="replace",
    ...
    for path in (line.strip().replace("\\", "/") for line in changed if line.strip()):
        if path in EPIC_277_ALLOWED_CHANGED_PATHS or path in EPIC_278_EXACT_PATHS:
            continue
        diff_parts.append(
            subprocess.run(
                ["git", "diff", "main", "--", path],
...
def assert_allowed_diff_only(root: Path) -> None:
    unexpected = []
    allowed_diff_paths = set(ALLOWED_DIFF_PREFIXES) | EPIC_276_EXACT_PATHS | EPIC_278_EXACT_PATHS
...
def assert_path_not_changed(root: Path, relative_path: str) -> None:
    ...
    if path in TASK_257_EXACT_PATHS or path in EPIC_276_EXACT_PATHS or path in EPIC_278_EXACT_PATHS:
        return
...
def test_no_runtime_sql(h: Harness) -> None:
    forbidden = subprocess.run(
        [
            ...
            *[
                f":(exclude){path}"
                for path in EPIC_279_EXACT_PATHS
                if path.startswith(("libs/DataServiceApi/", "apps/", "libs/ShellServices/", "libs/ShellCore/"))
            ],
        ],
        cwd=h.root,
        check=True,
...
def test_production_qml_unmodified(h: Harness) -> None:
    allowed_task200_qml = {"apps/ETFDecisionShell/qml/pages/ShellAccountingReadOnlyPage.qml"} | EPIC_287_GLOBAL_CHINESE_UI_READABILITY_EXACT_PATHS | EPIC_289_FIX_EXACT_PATHS
    for path in changed_paths(h.root):
        require(
...
def test_accountingengine_replay_unmodified(h: Harness) -> None:
    for path in changed_paths(h.root):
        require(not path.startswith("libs/AccountingEngine/") or path in EPIC_276_EXACT_PATHS, "AccountingEngine must not change outside EPIC-276 read-only replay paths")
...
def test_no_tradedraft_suggestion(h: Harness) -> None:
    diff = diff_text_excluding_epic277(h.root, "libs", "apps")
...
ALLOWED_DIFF_PREFIXES.update(TASK_248_FULL_CTEST_SELF_CONSISTENCY_PATHS)

ALLOWED_DIFF_PREFIXES.update({
