...
{
    return root / "docs" / "84_shell_accounting_replay_snapshot_rebuild_gate.md";
}

std::filesystem::path docs85Path(const std::filesystem::path& root)
{
    return root / "docs" / "85_shell_accounting_replay_snapshot_rebuild_test_plan.md";
}
...
std::vector<std::string> uiDirectForbiddenTokens()
{
    return {
        "AccountingEngine",
...
std::vector<std::string> writeSqlForbiddenTokens()
{
    return {
        "INSERT INTO",
        "UPDATE ",
