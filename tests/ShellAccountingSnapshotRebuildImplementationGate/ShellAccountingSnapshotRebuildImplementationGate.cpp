...
namespace etfdt::tests::shell_accounting_snapshot_rebuild_implementation_gate {
namespace {
...
std::filesystem::path docs89Path(const std::filesystem::path& root)
{
    return root / "docs" / "89_shell_accounting_snapshot_rebuild_implementation_gate.md";
}

std::filesystem::path docs90Path(const std::filesystem::path& root)
{
    return root / "docs" / "90_shell_accounting_snapshot_rebuild_implementation_test_plan.md";
}
...
std::vector<std::string> writeForbiddenTokens()
{
    return {
...
std::vector<std::string> uiTriggeredRebuildTokens()
{
    return {
        "rebuildSnapshot",
        "rebuildFromReplay",
