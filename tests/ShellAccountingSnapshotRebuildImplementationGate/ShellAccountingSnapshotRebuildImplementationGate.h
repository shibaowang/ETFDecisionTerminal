...
[[nodiscard]] std::filesystem::path docs89Path(const std::filesystem::path& root);
[[nodiscard]] std::filesystem::path docs90Path(const std::filesystem::path& root);
...
[[nodiscard]] std::vector<std::string> writeForbiddenTokens();
[[nodiscard]] std::vector<std::string> uiTriggeredRebuildTokens();
